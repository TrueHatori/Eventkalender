 ...
 *
 * $Source: /cvs_backup/e107_0.8/e107_plugins/eventcalender/e_sitelink.php,v $
 * $Revision$
 * $Date$
 * $Author$
 *
...
class eventcalender_sitelink // include plugin-folder in the name.
{
	function config()
	{
		global $pref;
	...
	function myCategories()
	{
		...
		$sql->select("eventcalender","*","eventcalender_id != '' ");
		while($row = $sql->fetch())
		{
			$sublinks[] = array(
				'link_name'			=> $tp->toHTML($row['eventcalender_name'],'','TITLE'),
				'link_url'			=> e107::url('eventcalender', 'other', $row),
				'link_description'	=> '',
				'link_button'		=> $row['eventcalender_icon'],
				'link_category'		=> '',
				'link_order'		=> '',
				'link_parent'		=> '',
				'link_open'			=> '',
